...
use sgrmath_pn::{shaders::ShaderOptions, Solution, PNP};

pub fn run_network(network_name: &str) -> io::Result<()> {
    ...
    let output = Command::new("python3")
        .arg(format!("./examples/py_scripts/{}.py", network_name))
        .arg(".temp/train.csv")
        .arg(".temp/test.csv")
        .arg("--inputs")
        .arg(format!("{}", solution.vectors.len() / solution.vectors_count + 10))
        .output()?;
...
fn process_csv(wgpu: &WgpuContext, filename: &str, solution: &Solution) {
    ...
    let pnp = PNP::from_csv(
        ...
        (solution.vectors.len() / solution.vectors_count) - 1, 
    ...
    let params = ProblemParams {
        ...
        solutions_offset: 0,
        solutions_count: 1,
        vector_length: solution.vectors.len() / solution.vectors_count,
